...
function addRow(tableType) {
  ...
  for (let i = 0; i < table.rows[0].cells.length - 1; i++) {
      ...
      cell.innerHTML = `<input type="text" class="input-field" placeholder="Enter value">`;
  }

  // Add the Actions column with buttons
  let actionsCell = row.insertCell(table.rows[0].cells.length - 1);
  actionsCell.classList.add('actions-cell'); // Class for styling
...
function editRow(button) {
  ...
  for (let i = 0; i < cells.length - 1; i++) {
      let cell = cells[i];
      cell.innerHTML = `<input type="text" class="input-field" value="${cell.innerText}">`;
  }
...
function saveRow(button) {
  ...
  for (let i = 0; i < cells.length - 1; i++) {
      ...
      if (input) {
          cell.innerHTML = input.value || "N/A"; // Default to "N/A" if no value is entered
      }
  }
...
// Function to update a row after editing
function updateRow(button) {
  ...
  for (let i = 0; i < cells.length - 1; i++) {
      ...
      let input = cell.querySelector('input');
      if (input) {
          cell.innerHTML = input.value || "N/A";
      }
  }
